...
curves_by_fips = function(
  ...
  # unique list of fips
  fips = substr(
    varnames[substr(varnames, 0, 18) == "b[(Intercept) fips"], 20, 24
  )
  ...
  for (f in fips) {
    ...
    interv_effect_decrease = samples[ ,nms] %*% t(poly_int)
    ...
    days_btwn = (
      f_data$days_btwn_decrease_thresh[1] + intervention_offset
    )
    interv_effect_decrease = interv_effect_decrease +
      samples[ ,nms] %*% t(poly_int * days_btwn)
    ...
    out = list(
      ...
      intervention_decrease_curve = interv_effect_decrease,
      random_effect_curve = resid,
      covar_curve = covar_effect,
      linear_predictor = (
        covar_effect + interv_effect + interv_effect_decrease + resid
      ),
...
nchs_effect_summaries = function(extracted_curves) {
  ...
  vars = c(
    ...
    "random_effect_curve",
    "covar_curve",
    "intervention_decrease_curve"
  ) 
    
  funs = map(vars, ~ {
    ans = list()
    var = .x
    ans[[paste0(var, "_mean")]] = function(x) apply(x[[var]], 2, mean)
    ans[[paste0(var, "_q50")]] = function(x) apply(x[[var]], 2, quantile, 0.5)
    ans[[paste0(var, "_q25")]] = function(x) apply(x[[var]], 2, quantile, 0.25)
    ans[[paste0(var, "_q75")]] = function(x) apply(x[[var]], 2, quantile, 0.75)
    ans[[paste0(var, "_q05")]] = function(x) apply(x[[var]], 2, quantile, 0.05)
    ans[[paste0(var, "_q95")]] = function(x) apply(x[[var]], 2, quantile, 0.95)
    return(ans)
  }) %>% 
    reduce(c)
...
main = function() {
  ...
  df = read_feather("../../county_train_decrease.feather")%>%
    select(days_since_thresh, y, nchs, college, age_65_plus, black,
           ...
           days_btwn_stayhome_thresh, pop, state, county,
           date, stayhome, decrease_50_total_visiting,
           threshold_day) %>%
    na.omit()
  ...
  peaks_pos = apply(nchs_summaries$linear_predictor_mean[ ,-1], 1, which.max) - 1
  # in terms of 100,000 people
  peaks_val = apply(1e6 * exp(nchs_summaries$linear_predictor_mean[ ,-1]), 1, max)
  
  peak_flatness = c()  # zero is very flat
  for (i in 1:6) {
    idx = peaks_pos[i] + 1
    x = as.numeric(nchs_summaries$linear_predictor_mean[i, -1])
  ...
  q50 = nchs_summaries$linear_predictor_q50 %>% 
    gather(t, median, -nchs) %>% 
    mutate(t=as.integer(substring(t, 2)))
  q95 = nchs_summaries$linear_predictor_q95 %>% 
    gather(t, q95, -nchs) %>%
    mutate(t=as.integer(substring(t, 2)))
  q05 = nchs_summaries$linear_predictor_q05 %>% 
    gather(t, q05, -nchs) %>%  
    mutate(t=as.integer(substring(t, 2)))
  ...
  # Note: [ ,-1] to remove nchs
  N = table(distinct(select(df, fips, nchs))$nchs)
  peaks_pos_late = apply(nchs_summaries$linear_predictor_mean[ ,-1], 1, which.max) - 1
  # in terms of 100,000 people
  peaks_val_late = apply(1e6 * exp(nchs_summaries$linear_predictor_mean[ ,-1]), 1, max)
  ...
  # Per capita plot per nchs
  q50_late = nchs_summaries$linear_predictor_q50 %>% 
    gather(t, median, -nchs) %>% 
    mutate(t=as.integer(substring(t, 2)))
  q95_late = nchs_summaries$linear_predictor_q95 %>% 
    gather(t, q95, -nchs) %>%
    mutate(t=as.integer(substring(t, 2)))
  q05_late = nchs_summaries$linear_predictor_q05 %>% 
    gather(t, q05, -nchs) %>%  
    mutate(t=as.integer(substring(t, 2)))
  ...
  # Note: [ ,-1] to remove nchs
  N = table(distinct(select(df, fips, nchs))$nchs)
  peaks_pos_early = apply(nchs_summaries$linear_predictor_mean[ ,-1], 1, which.max) - 1
  # in terms of 100,000 people
  peaks_val_early = apply(1e6 * exp(nchs_summaries$linear_predictor_mean[ ,-1]), 1, max)
  ...
  # Per capita plot per nchs
  q50_early = nchs_summaries$linear_predictor_q50 %>% 
    gather(t, median, -nchs) %>% 
    mutate(t=as.integer(substring(t, 2)))
  q95_early = nchs_summaries$linear_predictor_q95 %>% 
    gather(t, q95, -nchs) %>%
    mutate(t=as.integer(substring(t, 2)))
  q05_early = nchs_summaries$linear_predictor_q05 %>% 
    gather(t, q05, -nchs) %>%  
    mutate(t=as.integer(substring(t, 2)))
  ...
  # Now aggregate all components and plot components
  nchs_summaries = nchs_effect_summaries(curve_summaries)
  ...
  for (v in names(vars)) {
    ...
    for (suffix in c("q95", "q05")) {
      ...
      tmp[[field]] = nchs_summaries[[field]] %>% 
        gather(t, value, -nchs) %>%
        mutate(t=as.integer(substring(t, 2))) %>% 
        rename_with(function(x) suffix, .cols=value) 
    }
    curves[[v]] = tmp %>% 
      reduce(left_join, by=c("nchs", "t")) %>% 
      filter(t <= 30) %>% 
      mutate(var=vars[v])
  }
  ...
  ggsave(
    "img/nchs_double_stick_effect.pdf",
    width=18,
    height=8,
    units="cm"
  )
