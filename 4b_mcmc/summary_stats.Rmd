...
```{r}
county_fit <- readRDS("./roll_deaths/county_future_fit.rds")
county_ctr1 <- readRDS("./roll_deaths/county_future_ctr1.rds")
county_ctr3 <- readRDS("./roll_deaths/county_future_ctr3.rds")
```
...
for(x in 1:6) {
  ...
  eff[["late_fit"]] <- apply(county_ctr1_effect[, county_idx] - 
                             county_fit_effect[, county_idx], 1,sum)
  eff_summary_ <- apply(sapply(eff, quantile, c(0.0, 0.025, 0.5, 0.5, 0.975, 1)), 1, function(x) x)
  #eff_summary_ <- apply(sapply(eff, summary), 1, function(x) x)
...
for (x in 1:6) {
  ...
  for (d in dates) {
    ...
    if(length(county_idx) > 1) {
      eff_fit[[as.character(d)]] <- apply(county_fit_effect[, county_idx], 1, sum)
      eff_late[[as.character(d)]] <- apply(county_ctr1_effect[, county_idx], 1, sum)
      eff_early[[as.character(d)]] <- apply(county_ctr3_effect[, county_idx], 1,sum)
    } else {
  ...
  eff_fit_summary_ <- as_tibble(apply(sapply(eff_fit, summary), 1, function(x) x)) %>% mutate(date = dates, nchs = x, type = "fit")
  eff_late_summary_ <- as_tibble(apply(sapply(eff_late, summary), 1, function(x) x)) %>% mutate(date = dates, nchs = x, type = "late")
  eff_early_summary_ <-  as_tibble(apply(sapply(eff_early, summary), 1, function(x) x)) %>% mutate(date = dates, nchs = x, type = "early")
...
names(eff_fit_summary) <- c("min", "q25", "median", "mu", 
                            "q75", "max", "date", "nchs", "type")
...
for(x in 1:6) {
  ...
  filter(nchs == x) %>% 
    ...
    geom_ribbon(aes(x=date, ymin=q25, ymax=q75., fill = type, alpha = 0.5)) + 
    labs(title = x)
}
plots
```
...
```{r}
county_fit <- readRDS("./roll_deaths_decrease/county_future_fit.rds")
county_ctr1 <- readRDS("./roll_deaths_decrease/county_future_ctr1.rds")
county_ctr3 <- readRDS("./roll_deaths_decrease/county_future_ctr3.rds")
```
...
```{r}
county_idx <- which(county_decrease$date == as.Date("2020-04-20") & 
