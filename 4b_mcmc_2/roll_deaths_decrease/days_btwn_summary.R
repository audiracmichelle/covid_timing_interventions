...
## Read county_train
county_pred <- read_feather("../../county_train_decrease_2.feather")
...
county_pred1 = county_pred %>% 
  ...
  mutate(intrv_decrease = as.numeric(date >= decrease_40_total_visiting + 12 + up))

county_pred3 = county_pred %>% 
  mutate(days_btwn_decrease_thresh = days_btwn_decrease_thresh + down) %>%
  mutate(intrv_decrease = as.numeric(date >= decrease_40_total_visiting + 12 + down))
