...
model_data = stan_input_data(county_train, type="decrease", order=1, lag=14)
...
saveRDS(fit, "models/11_linear.rds")
# saveRDS(fit2, "models/1_basemodel_mcmc.rds")
fit = readRDS("models/11_linear.rds")
...
predicted = my_posterior_predict(fit, county_train, type="decrease", order=1, lag=14, eval_pre = TRUE)
...
# $summary
# mean se_mean       sd      2.5%
# beta_covars_post[1,1] -9.636239     NaN 0.489791 -9.978674
# 25%       50%       75%     97.5%
#   beta_covars_post[1,1] -9.906152 -9.790425 -9.587602 -8.264535
# n_eff     khat
# beta_covars_post[1,1]   NaN 16.72898