...
model = rstan::stan_model("../8_temporal_saturated.stan")

# first run with variational inference,
# it should take ~ 10 mins for default tol (0.01) and ~40min for 50k iters
# model_data$scale_time_eff = 4.0
...
saveRDS(fit, paste("./models/8_temporal_saturated.rds", sep = ""))
fit = readRDS("./models/8_temporal_saturated.rds")

# revised_2 uses the joint dataset with min cum deahts >= 1
# saveRDS(fit2, "./models/8_temporal_saturated_mcmc.rds")
# fit = readRDS("./models/8_temporal_saturated_mcmc.rds")
...
predicted = my_posterior_predict(fit, county_eval, type="decrease", lag=14, states=TRUE, temporal=FALSE, eval_pre = TRUE)
...
hist(autocor, col=alpha("blue", 0.5), main="autocor posterior")
