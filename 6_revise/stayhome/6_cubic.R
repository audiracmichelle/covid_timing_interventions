...
model_data = stan_input_data_cubic(county_train, type="decrease", lag=14)

model = rstan::stan_model("../6_cubic.stan")
...
saveRDS(fit, paste("./6_cubic.rds", sep = ""))
fit = readRDS("./6_cubic.rds")
...
predicted = my_posterior_predict(fit, county_eval, type="decrease", lag=14, cubic=TRUE, eval_pre = TRUE)
...
ggplot(plotdata) +
  ...
  theme_minimal() +
  labs(
    title=sprintf("Cubic Polynomial Bent prediction FIPS %s", f1),
    subtitle="How is the bent in NY predicted\nif we don't include NY post intervention in the data?"
  ) +
