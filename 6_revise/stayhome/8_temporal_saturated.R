...
saveRDS(fit, paste("./8_temporal_saturated.rds", sep = ""))
fit = readRDS("./8_temporal_saturated.rds")
...
predicted = my_posterior_predict(fit, county_eval, type="decrease", lag=14, states=TRUE, saturated_time=TRUE, eval_pre = TRUE)
...
overdisp = rstan::extract(fit, pars="autocor")$autocor
hist(overdisp, col=alpha("blue", 0.5), main="autocor posterior")
