...
## Read county_train
county_train <- read_feather("../../county_train_.feather")
model_data = stan_input_data(county_train)
...
dbtwn = dbtwn[dbtwn$days_since_intrv_ >= 0, ]
dbtwn = dbtwn$days_since_thresh[1]
abline(v=dbtwn, lty=3, col="gray")
title(sprintf("FIPS %s", f1))


predicted = my_posterior_predict(fit, county_train)
