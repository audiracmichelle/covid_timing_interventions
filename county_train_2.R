...
county_train %<>%  
  mutate(y = roll_deaths,  
         intrv_stayhome = (date - schools >= 12) * 1,  
         intrv_decrease = (date - decrease_50_school_visits >= 12) * 1,
         days_since_intrv_stayhome = as.numeric(date - schools - 12 + 1), 
         ...
         days_btwn_stayhome_thresh = as.numeric(schools - threshold_day), 
...
county_train_stayhome <- county_train %>% 
  filter(!is.na(schools), days_since_intrv_stayhome <= 17)
...
write_feather(county_train_stayhome, "./county_train_stayhome_2.feather")
write_feather(county_train_decrease, "./county_train_decrease_2.feather")
