...
```{r}
# all counties in county_cases_desc reached the cases threshold
county_map <- county_deaths_desc %>% 
  mutate(no_thresh = as.factor(is.na(threshold_day)))

table(county_map$no_thresh)
...
```{r}
plot_usmap(data = county_cases_desc, 
           include = county_cases_desc$fips, 
           values = "diff_thresh") +
  theme(legend.position = "right") + 
  theme(legend.title = element_blank()) + 
...
```{r}
county_cases_desc %>% 
  mutate(days_betwn_decrease_stayhome = as.numeric(decrease_50_total_visiting - stayhome)) %>% 
  ggplot() +
  geom_density(aes(x = days_betwn_decrease_stayhome, fill = nchs), alpha = 0.5)
...
```{r}
county_cases_desc %>% 
  ggplot(aes(x = threshold_day, fill = nchs)) + 
  geom_bar()
...
```{r}
plot_usmap(data = county_cases_desc, include = county_cases_desc$fips, 
...
```{r}
county_cases_desc %>%
  select(decrease_50_school_visits,
         decrease_50_recreation_visiting,
         decrease_50_total_visiting) %>% lapply(summary)
```

```{r}
county_cases_desc %>%
  ggplot() +
  geom_density(aes(x = days_btwn_decrease_thresh, fill = nchs), alpha = 0.5)
...
```{r}
plot_usmap(data = county_cases_desc, include = county_cases_desc$fips, 
           values = "days_btwn_decrease_thresh") +
  theme(legend.position = "right") + 
  theme(legend.title = element_blank()) + 
  scale_fill_gradient2(low = "blue", midpoint = 0, high = "red")
...
quantile(cum_deaths_, 0.5)
```

```{r}
xx <- county_train %>% filter(index == 1)
table(is.na(xx$stayhome))
table(is.na(xx$decrease_50_total_visiting))
table(is.na(xx$stayhome) | is.na(xx$decrease_50_total_visiting))
```
